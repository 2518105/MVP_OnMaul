  ...
  useEffect(() => {
    function apply(t) {
      ...
      topRef.current.style.transform =
        `translate3d(${lerp(-SPREAD,0,e1).toFixed(1)}px,${(-gap*e1).toFixed(1)}px,${Z_TOP}px) rotateZ(${leanD}deg) rotateX(${tilt}deg)`;
      botRef.current.style.transform =
        `translate3d(${lerp(SPREAD,0,e1).toFixed(1)}px,${(gap*e1).toFixed(1)}px,${Z_BOT}px) rotateZ(${leanD}deg) rotateX(${tilt}deg)`;
      midRef.current.style.transform =
        `translate3d(0,0,${Z_MID}px) rotateZ(${leanD}deg) rotateX(${tilt}deg) rotateZ(${lerp(0,45,e1).toFixed(1)}deg)`;
    }
