...
class User(Base):
    ...
    user_type = Column(Enum(UserType, native_enum=False), default=UserType.immigrant, nullable=False)
...
class Notice(Base):
    ...
    content = Column(Text, nullable=False)
    category = Column(Enum(NoticeCategory, native_enum=False), nullable=False)
    author_id = Column(Integer, ForeignKey("users.id"), nullable=False)
...
class CalendarEvent(Base):
    ...
    event_type = Column(Enum(ScheduleType, native_enum=False), nullable=False)
    author_id = Column(Integer, ForeignKey("users.id"), nullable=False)
    created_at = Column(DateTime, default=datetime.utcnow)
